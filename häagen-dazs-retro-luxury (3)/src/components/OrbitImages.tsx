...
export default function OrbitImages({
  ...
  const items = images.map((src, index) => (
    <div key={src} className="relative group w-full h-full">
      {/* Outer ambient glow backing ring */}
      <div className="absolute inset-0 bg-[#ffd7bf]/10 rounded-full blur-xs group-hover:bg-[#ffd7bf]/20 transition-all duration-300" />
      <img
        src={src}
        alt={`${altPrefix} ${index + 1}`}
        draggable={false}
        className="orbit-image rounded-full hover:scale-110 active:scale-95 transition-all duration-500 shadow-xl border-3 border-[#ffdbc9]/40 hover:border-[#ffdbc9] bg-[#fffdfc] p-0.5 object-cover"
